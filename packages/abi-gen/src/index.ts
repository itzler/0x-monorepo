...
function registerPythonHelpers(): void {
    ...
    Handlebars.registerHelper('disableLineTooLongLintIfTuple', (inputType: string) =>
        inputType === 'tuple' ? '  # noqa: E501 (line too long)' : '',
    );
        ...
        for (const pythonTupleName in tupleDefinitions) {
            if (tupleDefinitions[pythonTupleName]) {
                tupleDeclarations.push(
                    `class ${pythonTupleName}(  # noqa: E501 (line too long)\n    TypedDict\n):\n    """Python representation of a tuple or struct."""${
